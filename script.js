...
function cambiarTipoAceleracion() {
  ...
  document.getElementById("aceleracion-constante").style.display =
    tipoSeleccionado === "constante" ? "block" : "none";
  document.getElementById("aceleracion-variable").style.display =
    tipoSeleccionado === "variable" ? "block" : "none";
...
function iniciarSimulacion() {
  ...
  const velocidadInicial = parseFloat(
    document.getElementById("velocidad").value
  );

  const auto = document.getElementById("auto");
  const anchoPista =
    document.querySelector(".contenedor-animacion").offsetWidth -
    auto.offsetWidth;
  ...
  intervalo = setInterval(() => {
    ...
    aceleracionMaxima = Math.max(
      Math.abs(aceleracionMaxima),
      Math.abs(aceleracionActual)
    ); // Usar Math.abs para aceleracionMaxima
    ...
    document.getElementById("velocidadActual").innerText =
      velocidadActual.toFixed(2);
    document.getElementById("aceleracionActual").innerText =
      aceleracionActual.toFixed(2);
...
function cambiarAceleracionTiempoReal(cambio) {
  ...
  if (tipoAceleracionSeleccionada === "constante") {
    ...
    document.getElementById(
      "aceleracion-actual"
    ).innerText = `Aceleración: ${aceleracionActual.toFixed(1)} m/s²`;
  } else {
    try {
      ...
      if (match) {
        ...
        document.getElementById("funcion-aceleracion").value =
          1 + cambio + "*(" + expresion + ")";

      document.getElementById(
        "aceleracion-actual"
      ).innerText = `Aceleración modificada`;
    } catch (error) {
...
function actualizarTipoMovimiento() {
  ...
  if (
    Math.abs(aceleracionActual) < 0.01 &&
    tipoAceleracionSeleccionada === "constante"
  )
...
function toggleDataset(index) {
  ...
  if (dataset.hidden) botones[index].classList.add("inactive");
  else botones[index].classList.remove("inactive");
}
...
function calcularIntegrales() {
  ...
  switch (tipoFuncion) {
    ...
    case "posicion":
      ...
      break;
    case "aceleracion":
      integralResultado = calcularIntegralTrapecio(
        datosTiempo,
        datosAceleracion
      );
      unidades = "m/s";
  ...
  resultados.innerHTML = `
        ...
        <div class="resultado-integral">
            <p><strong>Función integrada:</strong> ${
              tipoFuncion.charAt(0).toUpperCase() + tipoFuncion.slice(1)
            }</p>
            <p><strong>Método:</strong> Regla del Trapecio</p>
            <p><strong>Intervalo:</strong> [0, ${datosTiempo[
              datosTiempo.length - 1
            ].toFixed(2)}] segundos</p>
            <p><strong>Valor de la integral:</strong> ${integralResultado.toFixed(
              4
            )} ${unidades}</p>
            <p><strong>Interpretación física:</strong> ${descripcionFisica}</p>
            <p><strong>Número de intervalos:</strong> ${
              datosTiempo.length - 1
            }</p>
        </div>
        ...
        <div class="detalles-calculo">
            ...
            <p>Where Δt = ${(datosTiempo[1] - datosTiempo[0]).toFixed(
              1
            )} seconds</p>
        </div>
    `;
...
function calcularIntegralTrapecio(tiempos, valores) {
  if (tiempos.length !== valores.length || tiempos.length < 2) return 0;
...
function calcularIntegralAnaliticamente() {
  ...
  else calcularIntegralAnaliticaVariable(tipoFuncion, resultados);
}
...
function calcularIntegralAnaliticaConstante(tipoFuncion, elementoResultados) {
  ...
  switch (tipoFuncion) {
    case "velocidad":
      integral =
        velocidadInicial * tiempoFinal +
        0.5 * aceleracion * Math.pow(tiempoFinal, 2);
    ...
    case "posicion":
      integral =
        0.5 * velocidadInicial * Math.pow(tiempoFinal, 2) +
        (1 / 6) * aceleracion * Math.pow(tiempoFinal, 3);
  ...
  elementoResultados.innerHTML = `
        ...
        <div class="resultado-integral">
            ...
            <p><strong>Evaluada en [0, ${tiempoFinal.toFixed(
              2
            )}]:</strong> ${integral.toFixed(4)} ${unidades}</p>
...
function calcularIntegralAnaliticaVariable(tipoFuncion, elementoResultados) {
  const funcionAceleracion = document.getElementById(
    "funcion-aceleracion"
  ).value;
  const velocidadInicial = parseFloat(
    document.getElementById("velocidad").value
  ...
  const { velocityExpression, positionExpression, success } =
    getSymbolicIntegratedFunctions(
      funcionAceleracion,
      velocidadInicial,
      posicionInicial
    );
  ...
  if (success) {
    ...
    elementoResultados.innerHTML = `
            ...
            <div class="resultado-integral">
                ...
                <p><strong>Evaluada en t = ${tiempoFinal.toFixed(
                  2
                )} s:</strong> ${valorEvaluado} ${unidades}</p>
                <p><strong>Interpretación física:</strong> ${descripcion}</p>
            </div>
        `;
  } else {
...
function isConstant(node, variable) {
  ...
  if (node.args) return node.args.every((arg) => isConstant(arg, variable));
  return false;
}
 ...
 */
function getSymbolicIntegratedFunctions(
  ...
  console.log("--------------------------------------------------");
  console.log(
    "Attempting symbolic integration for acceleration expression:",
    accelerationExpression
  );
  ...
  try {
    ...
    console.log(
      "  Parsed acceleration node (a(t)):",
      nodeAceleracion.toString()
    );

    const integratedAcelerationNode = integrateNode(
      nodeAceleracion,
      "t",
      "accel_integration"
    ); // Añadir tag de origen
    console.log(
      "  Integrated acceleration node:",
      integratedAcelerationNode ? integratedAcelerationNode.toString() : "NULL"
    );

    if (integratedAcelerationNode) {
      let simplifiedIntegratedAceleration = math
        .simplify(integratedAcelerationNode)
        .toString();
      if (simplifiedIntegratedAceleration.trim() === "")
        simplifiedIntegratedAceleration = "0";

      console.log(
        "  Simplified integrated acceleration:",
        simplifiedIntegratedAceleration
      );
      ...
      formattedVelocityExpression = formattedVelocityExpression.replace(
        /unaryminus/g,
        "-"
      );
      ...
      console.log(
        "  Starting integration for velocity expression:",
        velocityExprNode.toString()
      );
      const integratedVelocityNode = integrateNode(
        velocityExprNode,
        "t",
        "vel_integration"
      ); // Añadir tag de origen
      console.log(
        "  Integrated velocity node:",
        integratedVelocityNode ? integratedVelocityNode.toString() : "NULL"
      );

      if (integratedVelocityNode) {
        let simplifiedIntegratedVelocity = math
          .simplify(integratedVelocityNode)
          .toString();
        if (simplifiedIntegratedVelocity.trim() === "")
          simplifiedIntegratedVelocity = "0";

        console.log(
          "  Simplified integrated velocity:",
          simplifiedIntegratedVelocity
        );
        ...
        formattedPositionExpression = formattedPositionExpression.replace(
          /unaryminus/g,
          "-"
        );
        ...
        console.log(
          "  integratedVelocityNode is NULL. successPosition remains false."
        );
    } else
      console.log(
        "  integratedAcelerationNode is NULL. successVelocity remains false."
      );
  } catch (e) {
    console.error(
      "[getSymbolicIntegratedFunctions] Error during symbolic integration process:",
      e
    );
  ...
  console.log(
    "Symbolic integration final success:",
    successVelocity && successPosition
  );
  console.log("--------------------------------------------------");
  return {
    velocityExpression: successVelocity
      ? velocityExpression
      : `v₀ + \\int a(t)dt`,
    positionExpression: successPosition
      ? positionExpression
      : `x₀ + \\int v(t)dt`,
...
function integrateNode(node, variable, context = "") {
  if (node.type === "ConstantNode") {
    if (node.value === 0) return new math.ConstantNode(0);
    return new math.OperatorNode("*", "multiply", [
      new math.ConstantNode(node.value),
      new math.SymbolNode(variable),
    ]);
    ...
    return new math.OperatorNode("*", "multiply", [
      ...
      new math.OperatorNode("^", "pow", [
        new math.SymbolNode(variable),
        new math.ConstantNode(newPower),
      ]),
    ]);
    ...
    if (node.op === "*" && node.args.length === 2) {
      ...
      if (constantPart && functionPart) {
        const integratedFunctionPart = integrateNode(
          functionPart,
          variable,
          context + "_mul_func"
        );
      ...
      const integratedArgs = node.args.map((arg) =>
        integrateNode(arg, variable, context + "_sum_arg")
      );
      if (integratedArgs.every((arg) => arg !== null)) {
        let newNode = integratedArgs[0];
        for (let i = 1; i < integratedArgs.length; i++) {
          newNode = new math.OperatorNode(
            node.op,
            node.op === "+" ? "add" : "subtract",
            [newNode, integratedArgs[i]]
          );
        }
        return newNode;
      } else return null;
    } else if (node.op === "unaryminus") {
      const integratedArg = integrateNode(
        node.args[0],
        variable,
        context + "_uminus_arg"
      );
      if (integratedArg) {
        const result = new math.OperatorNode("unaryminus", "unaryminus", [
          integratedArg,
        ]);
        return result;
      } else return null;
      ...
      if (isConstant(node.args[0], variable))
        ...
        const invertedConstant = new math.OperatorNode("/", "divide", [
          new math.ConstantNode(1),
          constantDivisor,
        ]);
        const multiplicationNode = new math.OperatorNode("*", "multiply", [
          functionNumerator,
          invertedConstant,
        ]);

        const result = integrateNode(
          multiplicationNode,
          variable,
          context + "_div_to_mul"
        );
    ...
    if (
      ...
      return new math.OperatorNode("unaryminus", "unaryminus", [
        new math.FunctionNode("cos", [new math.SymbolNode(variable)]),
      ]);
    else if (
      node.name === "cos" &&
      node.args.length === 1 &&
...
function mostrarEcuaciones() {
  ...
  if (tipoAceleracionSeleccionada === "constante") {
    ...
    if (Math.abs(aceleracion) < 0.01) {
      ecuacionesTexto = `
                ...
                <p><strong>Para t = ${tiempoFinal.toFixed(
                  2
                )} s:</strong> x(${tiempoFinal.toFixed(
        2
      )}) = ${posicionActual.toFixed(2)} m</p>
      ...
      ecuacionesTexto = `
        ...
        2
                ...
                <p><strong>Para t = ${tiempoFinal.toFixed(
                  2
                )} s:</strong> v(${tiempoFinal.toFixed(
        2
      )}) = ${velocidadFinal.toFixed(2)} m/s</p>
    ...
    if (success) {
      const displayVelocityExpression = velocityExpression
        .replace(/v_0/g, "v₀")
        .replace(/x_0/g, "x₀");
      const displayPositionExpression = positionExpression
        .replace(/v_0/g, "v₀")
        .replace(/x_0/g, "x₀");
      ...
      ecuacionesTexto += `
                ...
                <p><strong>Velocidad final calculada (numéricamente):</strong> v = ${velocidadFinal.toFixed(
                  2
                )} m/s</p>

                <p><strong>Ecuación de posición:</strong> x(t) = x₀ + \\int v(t)dt</p>
                <p><strong>Posición final calculada (numéricamente):</strong> x = ${posicionActual.toFixed(
                  2
                )} m</p>
  ...
  ecuacionesTexto += `
        ...
        <p><strong>Distancia recorrida:</strong> ${posicionActual.toFixed(
          2
        )} m</p>
        <p><strong>Velocidad inicial:</strong> ${velocidadInicial} m/s</p>
        <p><strong>Velocidad final:</strong> ${velocidadFinal.toFixed(
          2
        )} m/s</p>
    `;
...
function calcularYMostrarEstadisticas() {
  const velocidadMedia =
    contadorMuestras > 0 ? sumaVelocidades / contadorMuestras : 0;
  const aceleracionMedia =
    contadorMuestras > 0 ? sumaAceleraciones / contadorMuestras : 0;

  document.getElementById("velocidadMedia").innerText =
    velocidadMedia.toFixed(2);
  document.getElementById("aceleracionMedia").innerText =
    aceleracionMedia.toFixed(2);
  document.getElementById("tiempoTotal").innerText = tiempoActual.toFixed(2);
  document.getElementById("distanciaRecorrida").innerText =
    posicionActual.toFixed(2);
  document.getElementById("velocidadMaxima").innerText =
    velocidadMaxima.toFixed(2);
  document.getElementById("aceleracionMaxima").innerText =
    aceleracionMaxima.toFixed(2);

  const tipoMovimientoActual =
    document.getElementById("tipoMovimiento").innerText;
  const tipoMovimientoEstadisticas = document.querySelector(
    "#estadisticas #tipoMovimiento"
  );
